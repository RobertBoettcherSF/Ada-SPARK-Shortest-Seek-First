...
procedure SSTF_Demo is
   ...
   begin
      ...
      for Demo_Num in 1..5 loop
         ...
         begin
            ...
            for I in 1..Demo.Request_Count loop
               Ada.Text_IO.Put(Track_Index'Image(Actual_Requests(I)) & " ");
            end loop;
            Ada.Text_IO.New_Line;
            
            Ada.Text_IO.Put("  Schedule: ");
            for I in 1..Demo.Request_Count loop
               Ada.Text_IO.Put(Track_Index'Image(Actual_Schedule(I)) & " ");
            end loop;
            Ada.Text_IO.New_Line;
            ...
            for I in 1..Demo.Request_Count loop
               if Actual_Schedule(I) >= Current_Head then
                  Total_Seek_Distance := Total_Seek_Distance + (Actual_Schedule(I) - Current_Head);
               else
                  Total_Seek_Distance := Total_Seek_Distance + (Current_Head - Actual_Schedule(I));
               end if;
               Current_Head := Actual_Schedule(I);
            end loop;
   ...
   procedure Run_Interactive_Demo is
      Initial_Head : Track_Index;
      Request_Count : Integer;
      Requests : Track_Array(1..Max_Demo_Requests);
      Schedule : Track_Array(1..Max_Demo_Requests);
      Total_Seek_Distance : Track_Index;
      Current_Head : Track_Index;
   begin
      ...
      loop
         begin
            Ada.Text_IO.Put("Enter number of requests (1-" & Integer'Image(Max_Demo_Requests) & "): ");
            Ada.Integer_Text_IO.Get(Request_Count);
            
            if Request_Count < 1 or Request_Count > Max_Demo_Requests then
               Ada.Text_IO.Put_Line("Please enter a number between 1 and " & Integer'Image(Max_Demo_Requests) & ".");
            else
               exit;
            end if;
         exception
      ...
      -- Calculate schedule
      declare
         Actual_Requests : Track_Array(1..Request_Count);
         Actual_Schedule : Track_Array(1..Request_Count);
      begin
         -- Copy requests
         for I in 1..Request_Count loop
            Actual_Requests(I) := Requests(I);
         end loop;
         
         -- Get schedule
         Get_SSTF_Schedule(Initial_Head, Actual_Requests, Actual_Schedule);
         ...
         for I in 1..Request_Count loop
            if Actual_Schedule(I) >= Current_Head then
               ...
               Ada.Text_IO.Put_Line("  " & Track_Index'Image(Current_Head) & " -> " & 
                                   Track_Index'Image(Actual_Schedule(I)) & 
                                   " (seek: " & Track_Index'Image(Actual_Schedule(I) - Current_Head) & ")");
            else
               Total_Seek_Distance := Total_Seek_Distance + (Current_Head - Actual_Schedule(I));
               Ada.Text_IO.Put_Line("  " & Track_Index'Image(Current_Head) & " -> " & 
                                   Track_Index'Image(Actual_Schedule(I)) & 
                                   " (seek: " & Track_Index'Image(Current_Head - Actual_Schedule(I)) & ")");
            end if;
            Current_Head := Actual_Schedule(I);
         end loop;
